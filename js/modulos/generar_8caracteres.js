...
export function generar_8caracteres() {   
  ...
  do {
    ...
    // MAYÚSCULAS
    // Si solo están marcadas las mayúsculas con un for de 8 vueltas crea un número aleatorio de entre 65 y 90 que son las mayúsculas en la tabla Ascii, los paso a mayúsculas de la tabla Ascii y vol anexando carácteres al string "contrasenya_may",
    if (mayusculas.checked && !minusculas.checked && !numeros.checked && !simbolos.checked) {
      let contrasenya_may = "";
      for (let i = 0; i < 8; i++) {
    ...
    // SÍMBOLOS
    // Con un bucle de 8 vueltas busca un número aleatorio de entre 0 y 11 ya que son los índices de la array de símbolos, guardamos el valor del indice aleatorio en una variable, los vamos concatenando y los mostramos juntos por pantalla.
    if (!numeros.checked && !minusculas.checked && !mayusculas.checked && simbolos.checked) {
    ...
    // Aquí es lo mismo pero con mayúsculas y minúsculas, el bucle for solo da 4 vueltas porque he decidido que tiene que haber 4 mayúsculas y 4 minúsculas. Luego la paso a array para desordenarla y la vuelvo a pasar a String para tener la contrasenya final.
    if (mayusculas.checked && minusculas.checked && !numeros.checked && !simbolos.checked) {
      let contrasenya_may = "";
      let contrasenya_mim = "";
      for (let i = 0; i < 4; i++) {
    ...
    // Igual que en los anteriores pero con mayúsculas minúsculas y números, he decidido que la contraseña tendrá que tener 3 mayúsculas, 3 minúsculas y 2 números, así que se harán 2 bucles, uno de 3 vueltas y uno de 2.
    if (mayusculas.checked && minusculas.checked && numeros.checked && !simbolos.checked) {
      let contrasenya_may = "";
      let contrasenya_min = "";
      let contrasenya_num = "";
    ...
    // Aquí he decidido que solo habrá 2 símbolos
    if (!mayusculas.checked && minusculas.checked && numeros.checked && simbolos.checked) {
      let contrasenya_mim = "";
      let contrasenya_num = "";
      let contrasenya_simb = "";
